...
PRODUCER_COMMAND="$dir_kafka/bin/kafka-console-producer.sh --batch-size $batch_size --broker-list localhost:9092 --topic $topicname --property \"parse.key=true\" --property \"key.separator=:\""
...
# Function to send messages to Kafka
 send_kafka_messagev2() {
    ...
    printf "$batched_message" | $dir_kafka/bin/kafka-console-producer.sh --broker-list localhost:9092 --topic $topicname --property "parse.key=true" --property "key.separator=:"
  }
